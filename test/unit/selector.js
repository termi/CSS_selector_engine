...
var $$ = function(selector, node) {
    return Array.prototype.slice.call(document.documentElement.querySelectorAll.call(node || document, selector))
};
...
test("element", function() {
	...
	equal( $$("").length, 0, "Empty selector returns an empty array" );
	equal( $$(" ").length, 0, "Empty selector returns an empty array" );
	equal( $$("\t").length, 0, "Empty selector returns an empty array" );
	var form = document.getElementById("form");
	ok( !matchNode( form, "" ), "Empty string passed to matchesSelector does not match" );

	ok( $$("*").length >= 30, "Select all" );
	var all = $$("*"), good = true;
	...
	equal( $$("param", obj1).length, 2, "Object/param as context" );

	deepEqual( $$("select", form), q("select1","select2","select3","select4","select5"), "Finding selects with a context." );

	// Check for unique-ness and sort order
	deepEqual( $$("p, div p"), $$("p"), "Check for duplicates: p, div p" );

	t( "Checking sort order", "h2, h1", ["qunit-header", "qunit-banner", "qunit-userAgent"] );
	t( "Checking sort order", "h2:first-of-type, h1:first-of-type", ["qunit-header", "qunit-banner"] );
	...
	deepEqual( $$("#idTest", lengthtest), q("idTest"), "Finding element with id of ID." );
	deepEqual( $$("[name='id']", lengthtest), q("idTest"), "Finding element with id of ID." );
	deepEqual( $$("input[id='idTest']", lengthtest), q("idTest"), "Finding elements with id of ID." );

	var siblingTest = document.getElementById("siblingTest");
	deepEqual( $$("div em", siblingTest), [], "Element-rooted QSA does not select based on document context" );
	deepEqual( $$("div em, div em, div em:not(div em)", siblingTest), [], "Element-rooted QSA does not select based on document context" );
	deepEqual( $$("div em, em\\,", siblingTest), [], "Escaped commas do not get treated with an id in element-rooted QSA" );
	...
	ok( !!$$("body div div div").length, "No stack or performance problems with large amounts of descendents" );
	ok( !!$$("body>div div div").length, "No stack or performance problems with large amounts of descendents" );
	html.remove();
});
...
test("XML Document Selectors", function() {
	...
	equal( $$("foo_bar", xml).length, 1, "Element Selector with underscore" );
	equal( $$(".component", xml).length, 1, "Class selector" );
	equal( $$("[class*=component]", xml).length, 1, "Attribute selector for class" );
	equal( $$("property[name=prop2]", xml).length, 1, "Attribute selector with name" );
	equal( $$("[name=prop2]", xml).length, 1, "Attribute selector with name" );
	equal( $$("#seite1", xml).length, 1, "Attribute selector with ID" );
	equal( $$("component#seite1", xml).length, 1, "Attribute selector with ID" );
	equal( $$.matches( "#seite1", $$("component", xml) ).length, 1, "Attribute selector filter with ID" );
	equal( $$("meta property thing", xml).length, 2, "Descendent selector and dir caching" );
	ok( matchNode( xml.lastChild, "soap\\:Envelope" ), "Check for namespaced element" );
});
...
test("broken", function() {
	...
	function broken( name, selector ) {
		raises(function() {
			...
			return e.message.indexOf("SYNTAX_ERR") >= 0;
		}, name + ": " + selector );
	}
...
test("id", function() {
	...
	deepEqual( $$( "> span", $$("#fiddle\\\\Foo")[0] ), q([ "fiddleSpan" ]), "Escaped ID as context" );
	fiddle.remove();
	...
	equal( $$("#tName1")[0].id, 'tName1', "ID selector with same value for a name attribute" );
	equal( $$("#tName2").length, 0, "ID selector non-existing but name attribute on an A tag" );
	a.remove();
	...
	deepEqual( $$("div#form", document.body), [], "ID selector within the context of another element" );
...
test("class", function() {
	...
	deepEqual( $$(".e", div), [ div.firstChild ], "Finding a second class." );

	div.lastChild.className = "e";

	deepEqual( $$(".e", div), [ div.firstChild, div.lastChild ], "Finding a modified class." );

	ok( !matchNode( div, ".null"), ".null does not match an element with no class" );
	ok( !matchNode( div.firstChild, ".null div"), ".null does not match an element with no class" );
	div.className = "null";
	ok( matchNode( div, ".null"), ".null matches element with class 'null'" );
	ok( matchNode( div.firstChild, ".null div"), "caching system respects DOM changes" );
	ok( !matchNode( document, ".foo" ), "testing class on document doesn't error" );
	ok( !matchNode( window, ".foo" ), "testing class on window doesn't error" );
});
...
test("name", function() {
	...
	var form = document.getElementById("form");
	deepEqual( $$("input[name=action]", form), q("text1"), "Name selector within the context of another element" );
	deepEqual( $$("input[name='foo[bar]']", form), q("hidden2"), "Name selector for grouped form element within the context of another element" );

	form = jQuery("<form><input name='id'/></form>").appendTo("body");
	equal( $$("input", form[0]).length, 1, "Make sure that rooted queries on forms (with possible expandos) work." );
	...
	equal( $$("[name=tName1]")[0], a[0], "Find elements that have similar IDs" );
	equal( $$("[name=tName2]")[0], a[1], "Find elements that have similar IDs" );
...
test("child and adjacent", function() {
	...
	deepEqual( $$(":scope ~ em", siblingFirst), q("siblingnext", "siblingthird"), "Element Preceded By with a context." );
	deepEqual( $$(":scope + em", siblingFirst), q("siblingnext"), "Element Directly Preceded By with a context." );
	deepEqual( $$(":scope ~ em:first-of-type", siblingFirst), q("siblingnext"), "Element Preceded By positional with a context." );

	var en = document.getElementById("en");
	deepEqual( $$(":scope + p, a", en), q("yahoo", "sap"), "Compound selector with context, beginning with sibling test." );
	deepEqual( $$("a,:scope + p", en), q("yahoo", "sap"), "Compound selector with context, containing sibling test." );
	...
	equal( $$("#listWithTabIndex").length, 1, "Parent div for next test is found via ID (#8310)" );
	equal( $$("#listWithTabIndex li:eq(2) ~ li").length, 1, "Find by general sibling combinator (#8310)" );
	equal( $$("#__$$__").length, 0, "Make sure the temporary id assigned by $$ is cleared out (#8310)" );
	equal( $$("#listWithTabIndex").length, 1, "Parent div for previous test is still found via ID (#8310)" );
	...
	deepEqual( $$("> :first", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
	deepEqual( $$("> :eq(0)", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
	deepEqual( $$("> *:first", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
...
test("attributes", function() {
	...
	deepEqual( $$( "input[data-comma='0,1']" ), [ document.getElementById("el12087") ], "Without context, single-quoted attribute containing ','" );
	deepEqual( $$( 'input[data-comma="0,1"]' ), [ document.getElementById("el12087") ], "Without context, double-quoted attribute containing ','" );
	deepEqual( $$( "input[data-comma='0,1']", document.getElementById("t12087") ), [ document.getElementById("el12087") ], "With context, single-quoted attribute containing ','" );
	deepEqual( $$( 'input[data-comma="0,1"]', document.getElementById("t12087") ), [ document.getElementById("el12087") ], "With context, double-quoted attribute containing ','" );
	...
	// check2.checked = true;
	// ok( !$$.matches("[checked]", [ check2 ] ), "Dynamic boolean attributes match when they should with $$.matches (#11115)" );
	...
	deepEqual( $$( "[xml\\:test]", div ), [ div.firstChild ], "Finding by attribute with escaped characters." );
	div = null;
});
...
test("pseudo - misc", function() {
	...
	ok( $$("#qunit-fixture :not(:has(:has(*)))").length, "All not grandparents" );
	...
	jQuery.each( [ "button", "submit", "reset" ], function( i, type ) {
		...
		ok( match( $$("#input_" + type)[0], ":" + type ), "Input Matches :" + type );
		ok( match( $$("#button_" + type)[0], ":" + type ), "Button Matches :" + type );
	});
	...
	deepEqual(
		$$( "[id='select1'] *:not(:last-child), [id='select2'] *:not(:last-child)", q("qunit-fixture")[0] ),
	...
	$$.selectors.filters.icontains = function( elem, i, match ) {
		return $$.getText( elem ).toLowerCase().indexOf( (match[3] || "").toLowerCase() ) > -1;
	};
	$$.selectors.setFilters.podium = function( elements, argument ) {
	...
	delete $$.selectors.filters.icontains;
	delete $$.selectors.setFilters.podium;
...
test("pseudo - position", function() {
	...
	deepEqual( $$( ":last", context ), q("jquery12526"), "Post-manipulation positional" );
	QUnit.reset();

	// $$ extension
	var oldPOS = $$.selectors.match["POS"];
	$$.selectors.match["POS"] = new RegExp( oldPOS.source.replace("first", "primary"), "gi" );
	$$.selectors.setFilters["primary"] = $$.selectors.setFilters["first"];
	t( "Extend $$'s POS selectors to rename first to primary", "div:primary", ["qunit-testrunner-toolbar"] );
	// Reset
	$$.selectors.match["POS"] = oldPOS;
	delete $$.selectors.setFilters["primary"];
});
...
test("caching", function() {
	expect( 1 );
	$$( ":not(code)", document.getElementById("ap") );
	deepEqual( $$( ":not(code)", document.getElementById("foo") ), q("sndp", "en", "yahoo", "sap", "anchor2", "simon"), "Reusing selector with new context" );
});
