...
define([ "require", "benchmark.js/benchmark", "domReady!", "text!selectors.small.css?" + +(new Date()), "data/checkJava" ],
function( require, Benchmark, document, selectors ) {
	...
	var // Used to indicate whether console profiling is begin run
		...
		engines = {
			...
			"querySelectorAll polyfill":      "document.querySelectorAll( s, d )"
		},
	...
	function buildTable() {
		...
		try {
			_get("perf-table-headers").innerHTML = headers;
			_get("perf-table-body").innerHTML = rows;
		}
		catch(e) {//FOR IE
			_get("perf-table").outerHTML = "<table class='table table-bordered'><thead id='perf-table-headers'>" + headers + "</thead><tbody id='perf-table-body'>" + rows + "</tbody></table>"
		}
	...
	function onStart( event ) {
		...
		var selectorElem = _get( "selector" + selectorIndex );
	...
	function onCycle( event ) {
		var i = firstTestedColumn(),
			len = i + numEngines,
			tableBody = _get("perf-table-body"),
	...
	function onComplete() {
		...
		var fastestHz, slowestHz, elem, attr, j, jlen, td, ret,
			...
			len = i + numEngines,
			selectorElem = _get( "selector" + selectorIndex ),
			tableBody = _get("perf-table-body"),
			tds = tableBody.getElementsByTagName("td"),
